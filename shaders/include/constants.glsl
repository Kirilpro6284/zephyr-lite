...
/*  
    ...
    const int colortex2Format = RGBA16F; // low-res volumetric fog
    ...
    const int colortex8Format = RGBA16; // water mask
    const int colortex9Format = RGBA32UI; // gbuffer data: albedo (8:8:8), blockId (8), normal (8:8), lightLevels (8:8), textureNormal (16:16), specularMap (8:8:8:8) (solid -> deferred)
    const int colortex10Format = RGBA8; // sun & moon geometry (skytextured -> deferred), post-processing data (temporal -> post)
    const int colortex12Format = R32UI; // previous frame depth buffer
    const int colortex16Format = RGBA16F; // voxy water
